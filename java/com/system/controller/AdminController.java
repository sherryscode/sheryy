...
public class AdminController {
    ...
    // 删除学生（将请求方式改为POST，或保持GET但确保前端调用匹配）
    @RequestMapping(value = "/removeStudent", method = {RequestMethod.GET}) // 若前端用GET则保持，若用POST则改为POST
    private String removeStudent(Integer id, HttpServletRequest request) throws Exception {
        if (id == null) {
            request.getSession().setAttribute("message", "学生ID不能为空");
            return "redirect:/admin/showStudent";
        }
        try {
            // 1. 先删除用户登录表记录
            ...
            request.getSession().setAttribute("message", "删除学生成功！");
        } catch (Exception e) {
    ...
    public String removeTeacher(Integer id, HttpServletRequest request) throws Exception {
        if (id == null) {
            //加入没有带教师id就进来的话就返回教师显示页面
            return "redirect:/admin/showTeacher"; // 修复这里：添加redirect前缀
        }
        try {
            teacherService.removeById(id);
            userloginService.removeByName(id.toString());
            request.getSession().setAttribute("message", "删除教师成功！");
        } catch (Exception e) {
            request.getSession().setAttribute("message", "删除失败：" + e.getMessage());
        }
    ...
    public String removeCourse(Integer id, HttpServletRequest request) throws Exception {
        if (id == null) {
            //加入没有带教师id就进来的话就返回教师显示页面
            return "redirect:/admin/showCourse"; // 修复这里：添加redirect前缀
        }
        try {
            courseService.removeById(id);
            request.getSession().setAttribute("message", "删除课程成功！");
        } catch (Exception e) {
            request.getSession().setAttribute("message", "删除失败：" + e.getMessage());
        }
    ...
    public String userPasswordRest(Userlogin userlogin, HttpServletRequest request) throws Exception {
        System.out.println("普通用户账号密码重置处理");
        Userlogin u = userloginService.findByName(userlogin.getUsername());
        if (u != null) {
            if (u.getRole() == 0) {
                request.getSession().setAttribute("message", "该账户为管理员账户，无法修改");
                return "admin/userPasswordRest";
            }
            u.setPassword(userlogin.getPassword());
            userloginService.updateByName(userlogin.getUsername(), u);
            request.getSession().setAttribute("message", "密码重置成功！");
        } else {
            request.getSession().setAttribute("message", "没找到该用户");
        }
